...
bool CNetwork::Connect(const char* serverAddr, unsigned short nPort)
{
    ...
    // char to LPCWSTR
    //
    const char* CharserverAddr = serverAddr;
    WCHAR LserverAddr[256];
    memset(LserverAddr,0,sizeof(LserverAddr));
    MultiByteToWideChar(CP_ACP,0,CharserverAddr,strlen(CharserverAddr)+1,LserverAddr,
            sizeof(LserverAddr)/sizeof(LserverAddr[0]));

    // First check if the address is a dotted number "A.B.C.D"
    if (InetPtonW(AF_INET, LserverAddr, &(sAddr.sin_addr)) == 0)
    {
        ...
        if (erradapt == ERROR_SUCCESS)
        {
            ...
            while (ifa)
            {
                if (ifa->Type == MIB_IF_TYPE_ETHERNET)
                {
                    ...
                    // char to LPCWSTR
                    //
                    char* CharIpAddressString = ifa->IpAddressList.IpAddress.String;
                    WCHAR LIpAddressString[256];
                    memset(LIpAddressString,0,sizeof(LIpAddressString));
                    MultiByteToWideChar(CP_ACP,0,CharIpAddressString,strlen(CharIpAddressString)+1,LIpAddressString,
                            sizeof(LIpAddressString)/sizeof(LIpAddressString[0]));
                    //
                    char* CharIpMaskString = ifa->IpAddressList.IpMask.String;
                    WCHAR LIpMaskString[256];
                    memset(LIpMaskString,0,sizeof(LIpMaskString));
                    MultiByteToWideChar(CP_ACP,0,CharIpMaskString,strlen(CharIpMaskString)+1,LIpMaskString,
                            sizeof(LIpMaskString)/sizeof(LIpMaskString[0]));
                    //LPCWSTR LIpMaskString = _T(ifa->IpAddressList.IpMask.String);
                    if (InetPtonW(AF_INET, LIpAddressString, &nIPaddr) == 0 ||
                        InetPtonW(AF_INET, LIpMaskString, &nIPmask) == 0)
                    {
                        return false;
                    }
    ...
    if (erradapt == ERROR_SUCCESS)
    {
        ...
        while(pNextAd)
        {
            if (pNextAd->Type == MIB_IF_TYPE_ETHERNET)
            {
                ...
                // char to LPCWSTR
                //
                char* CharIpAddressString = pNextAd->IpAddressList.IpAddress.String;
                WCHAR LpNextAdIpAddressString[256];
                memset(LpNextAdIpAddressString,0,sizeof(LpNextAdIpAddressString));
                MultiByteToWideChar(CP_ACP,0,CharIpAddressString,strlen(CharIpAddressString)+1,LpNextAdIpAddressString,
                        sizeof(LpNextAdIpAddressString)/sizeof(LpNextAdIpAddressString[0]));
                if (InetPtonW(AF_INET, LpNextAdIpAddressString, &addr) != 0)
                {
                    return addr == nAddr;
                }
            }
